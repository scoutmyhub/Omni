...
void shoot_init(void)
{
  ...
  // 低通滤波器的初始化
  Low_Pass_Filter_Init(&ShootMotor_Left, 2, 1.2);
  Low_Pass_Filter_Init(&ShootMotor_Right, 2, 1.2);
...
//  PID_calc(&FireRightMoment, motor_shoot[1].given_current, Fire_right_motor_pid.out);
  ...
  CAN_cmd_shoot(shoot_CAN_Set_Current, 0, (int16_t)Fire_right_motor_pid.out, 0);
  CAN_cmd_shoot2(0, 0, (int16_t)Fire_left_motor_pid.out, 0);
}
...
void shoot_set_mode(void)
{
  ...
  if (ShootControl.RC_ctrl->mouse.press_l )//|| (switch_is_down(ShootControl.RC_ctrl->rc.s[Shoot_RC_Channel_left])))
  {
...
static void trigger_motor_turn_back(void)
{
  if (ShootControl.Turn_Back_Data.block_time > 200)
  {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM6)
  {
    static int8_t LastMAGAZINE;
	  ...
	  {
		__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_1, 300);
	  }
	  else 
	  {
