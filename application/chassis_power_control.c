...
float Accable(void);
float PIDGainPar[3] = {7500.0, 0.0, 50.0};

void ChassisGainPID_Init()
{ 
...
void ChassisPowerLimit(void)
{
    ...
    if(ChassisPowerLMT.RCData->rc.ch[0] <= 10 && ChassisPowerLMT.RCData->rc.ch[1] <=10 && ChassisPowerLMT.RCData->rc.ch[2])
    {
        for(i=0; i<4; i++)
        {
            ChassisGain[i] = 0.;
...
void PowerLimit(void)
{
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
        ...
        double real_val = (double)ChassisPowerLMT.ChassisData->motor_chassis[i].chassis_motor_measure->speed_rpm;
        a += pow(cmd_effort, 2);
