...
uint16_t LAST_CHASSIS_ROTATION_ANG = 0;
uint16_t CHASSIS_ROTATION_ANG_T = 0;
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    if(chassis_move_mode->chassis_RC->key.v & CHASSIS_ROTATION_ANG && !(LAST_CHASSIS_ROTATION_ANG & CHASSIS_ROTATION_ANG))
    {
        CHASSIS_ROTATION_ANG_T = !CHASSIS_ROTATION_ANG_T;
    }
    LAST_CHASSIS_ROTATION_ANG = chassis_move_mode->chassis_RC->key.v;
    //remote control  set chassis behaviour mode
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) && !CHASSIS_TOP_KEY_T && !CHASSIS_ROTATION_ANG_T)
    {
    ...
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) && !CHASSIS_TOP_KEY_T && !CHASSIS_ROTATION_ANG_T)
    {
    ...
    else if(CHASSIS_ROTATION_ANG_T)
    {
        chassis_behaviour_mode = CHASSIS_ROT;
    }
    ...
    else if (chassis_behaviour_mode == CHASSIS_ROT)
    {
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_ROT;
    }
}
